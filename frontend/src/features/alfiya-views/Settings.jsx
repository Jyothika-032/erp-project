...
const Settings = () => {
  ...
  useEffect(() => {
    const fetchLogs = async () => {
      try {
        const data = await apiClient.get('/merge-logs'); // Placeholder for config change logs
  ...
  const columns = [
    ...
    { 
      header: 'Value', 
      accessor: 'details',
      render: (row) => <span className="text-[10px] font-medium text-slate-400 truncate max-w-xs">{row.details}</span> 
    },
    { 
      header: 'Modified By', 
      accessor: 'status', 
    ...
    { 
      header: 'Timestamp', 
      accessor: 'created_at',
      render: (row) => <span className="text-[10px] font-bold text-slate-300">{new Date(row.created_at).toLocaleTimeString()}</span>
    }
  ];
