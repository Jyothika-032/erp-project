...
const Institutions = () => {
  ...
  const columns = [
    { 
      ...
      accessor: 'name',
      render: (row) => {
        const nameStr = row.name || row.institution_name || 'N/A';
        const idStr = row.id || row.institution_id || 'N/A';
        return (
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-slate-800">{nameStr}</p>
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{idStr}</p>
          </div>
        </div>
      )}
    },
  ...
  const filteredData = institutions.filter(inst => {
    const nameStr = inst.name || inst.institution_name || '';
    const idStr = String(inst.id || inst.institution_id || '');
